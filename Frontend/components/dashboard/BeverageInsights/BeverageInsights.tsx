...
export default function BeverageInsights() {
  ...
  const analysisCards = [
    {
      id: "liquor",
      icon: Wine,
      title: t("liquorCostAnalysis"),
      iconBg: "bg-green-500",
      samplePrompt: t("bevLiquorSample"),
      features: [
        t("varianceAnalysis"),
        t("costPerOunceTracking"),
        t("wasteIdentification"),
      ],
    },
    {
      id: "bar",
      icon: Package,
      title: t("barInventoryManagement"),
      iconBg: "bg-purple-500",
      samplePrompt: t("bevBarSample"),
      features: [
        t("stockLevelTracking"),
        t("reorderOptimization"),
    ...
    },
    {
      id: "pricing",
      icon: TrendingUp,
      title: t("beveragePricing"),
      iconBg: "bg-cyan-500",
      samplePrompt: t("bevPricingSample"),
      features: [
        t("marginAnalysis"),
  ...
  const handleCsvUpload = async (file: File) => {
    ...
    setMessages((prev) => [
      ...prev,
      { id: baseId, type: "user", text: `${t("uploadedCsv")} ${file.name}` },
    ])
              ...
              return (
                ...
                >
                  <div className="flex flex-col items-center text-center">
                    ...
                    <h3 className={cn("text-lg font-semibold mb-4", isSelected ? "text-white" : "text-gray-900 dark:text-white")}>
                      {card.title}
                    </h3>
                    <ul className="space-y-2 text-sm">
                      {card.features.map((feature, idx) => (
                        <li key={idx} className={cn("flex items-start gap-2", isSelected ? "text-gray-300" : "text-gray-600 dark:text-gray-400")}>
                          <CheckCircle2 className={cn("size-5 mt-0.5 flex-shrink-0", isSelected ? "text-cyan-400" : "text-cyan-500")} />
          ...
          <div className="space-y-4 pb-4 lg:min-h-[88px] xl:min-h-80">
            {messages.map((message) => (
              <div key={message.id} className={cn("flex gap-3", message.type === "user" ? "justify-end" : "justify-start")}>
                ...
                <div className={cn("max-w-3xl rounded-2xl", message.type === "user" ? "px-6 py-4 bg-gradient-to-br from-[#9810FA] to-[#155DFC] text-white" : message.html && (message.html.includes('class=\"report') || message.html.includes("class='report")) ? "p-0 bg-transparent" : "px-6 py-4 bg-gray-100 dark:bg-[#1E2939] text-gray-900 dark:text-white")}>
                  {message.type === "ai" && message.html ? (
                    <div id={`report-${message.id}`} className="relative w-full">
                      {(message.html.includes("class=\"report") || message.html.includes("class='report") || message.html.includes("report__header")) && (
                        <DownloadPdfButton targetId={`report-${message.id}`} filename="beverage_insights.pdf" />
                      )}
                      <div className={cn(styles.beverageHtml, "prose prose-sm max-w-none whitespace-normal dark:prose-invert")} dangerouslySetInnerHTML={{ __html: sanitizeBeverageHtml(message.html) }} />
                    </div>
                  ) : (
      ...
      <div className="sticky bottom-0 border-t border-gray-200 dark:border-gray-800 bg-white dark:bg-black">
        <div className="max-w-5xl mx-auto p-4">
          <div className="flex gap-2 items-center bg-gray-100 dark:bg-[#1E2939] rounded-xl px-4 py-3 border border-gray-300 dark:border-gray-700">
            ...
            />
            <Button variant="ghost" size="icon" className="text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300" onClick={() => fileInputRef.current?.click()} disabled={isLoading}>
            ...
            <Textarea
              ...
              placeholder={t("beverageInputPlaceholder")}
              ...
              disabled={isLoading}
            />
            <Button onClick={handleSendMessage} size="icon" className="bg-blue-600 hover:bg-blue-700 text-white rounded-lg" disabled={isLoading}>
              <Send className="w-4 h-4" />
            </Button>
          </div>
          <p className="text-center text-xs text-gray-500 mt-2">{t("pressEnterToSend")}</p>
        </div>
      </div>
    </div>
  )
